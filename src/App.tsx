...
function App() {
  ...
  const modules = [
    { num: 1, name: 'The Agentic Revolution', duration: '45 min', icon: '🤖' },
    { num: 2, name: 'The Tech Stack of 2026', duration: '45 min', icon: '⚡' },
    { num: 3, name: 'Deep Dive into OpenClaw', duration: '60 min', icon: '🔧' },
    { num: 4, name: 'Strategy, Ethics & Application', duration: '30 min', icon: '🎯' },
    { num: 5, name: 'Knowledge Check & Practice', duration: 'Variable', icon: '✅' },
  ]
  ...
  return (
    <div style={{ background: `linear-gradient(180deg, ${COLORS.paleCream} 0%, ${COLORS.softApricot} 40%, ${COLORS.burntOrange} 100%)`, minHeight: '100vh', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <section style={{ textAlign: 'center', padding: '60px 20px 80px', maxWidth: 900, margin: '0 auto' }}>
        <div style={{ display: 'inline-block', background: COLORS.rosePink, color: COLORS.pureWhite, padding: '6px 16px', borderRadius: 20, fontSize: 14, fontWeight: 600, marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
          <a href="#pricing" style={{ background: COLORS.rosePink, color: COLORS.pureWhite, padding: '16px 36px', borderRadius: 30, fontWeight: 700, fontSize: 18, textDecoration: 'none', boxShadow: `0 4px 20px ${COLORS.rosePink}40` }}>
            Start Learning →
          </a>
          <a href="#course" style={{ background: COLORS.slateCharcoal, color: COLORS.pureWhite, padding: '16px 36px', borderRadius: 30, fontWeight: 700, fontSize: 18, textDecoration: 'none' }}>
            View Course
          </a>
        </div>
      </section>
      ...
      <section id="course" style={{ background: COLORS.pureWhite, padding: '80px 20px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 20, marginBottom: 60 }}>
            {modules.map(m => (
              <div key={m.num} style={{ background: COLORS.paleCream, borderRadius: 16, padding: 24, textAlign: 'center' }}>
                ...
                <p style={{ color: COLORS.buildingTeal, fontSize: 14 }}>{m.duration}</p>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 32 }}>
            {[
              { icon: '🎓', title: 'Live Training', desc: '2 cohort sessions per week with real-time Q&A' },
              { icon: '🤖', title: 'Your Agent Team', desc: 'Build and keep working AI agents for your business' },
              { icon: '📱', title: 'Own Your Install', desc: "OpenClaw on your laptop — it's yours forever" },
              { icon: '📊', title: 'Interactive Learning', desc: 'Flashcards, quizzes, progress tracking' },
              { icon: '👥', title: 'Cohort Community', desc: 'Network with others on the same journey' },
              { icon: '🏆', title: 'Certificate', desc: 'Prove your AI orchestration skills' },
            ].map((f, i) => (
          ...
          <div style={{ display: 'grid', gap: 24 }}>
            {[
              ...
              <div key={i} style={{ display: 'flex', gap: 24, alignItems: 'center', background: 'rgba(255,255,255,0.05)', borderRadius: 16, padding: 24 }}>
                ...
                <div style={{ background: 'rgba(240,131,157,0.2)', color: COLORS.rosePink, padding: '6px 14px', borderRadius: 20, fontSize: 13, fontWeight: 600, flexShrink: 0 }}>
                  {s.time}
                </div>
              </div>
            ))}
              ...
              <div key={tier.id} onClick={() => setSelectedTier(tier.id)} style={{ 
                ...
                {tier.badge && (
                  <div style={{ 
                    ...
                    padding: '6px 40px', fontSize: 12, fontWeight: 700, transform: 'rotate(45deg)' 
                  }}>
                    {tier.badge}
                  </div>
                )}
                ...
                >
                  {selectedTier === tier.id ? '✓ Selected' : 'Enroll Now'}
                </button>
              </div>
            ))}
          </div>

          <p style={{ textAlign: 'center', marginTop: 32, color: COLORS.midGrey, fontSize: 14 }}>
          ...
          {[
            ...
            <details key={i} style={{ borderBottom: '1px solid #eee', padding: '20px 0' }}>
              <summary style={{ cursor: 'pointer', fontWeight: 700, color: COLORS.headlineBlack, fontSize: 18, listStyle: 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                {faq.q}
                <span style={{ color: COLORS.rosePink }}>▼</span>
              </summary>
      ...
      <section style={{ padding: '80px 20px', background: `linear-gradient(135deg, ${COLORS.slateCharcoal} 0%, #1a2a33 100%)`, textAlign: 'center' }}>
        ...
        <a href="#pricing" style={{ background: COLORS.rosePink, color: COLORS.pureWhite, padding: '20px 48px', borderRadius: 30, fontWeight: 800, fontSize: 20, textDecoration: 'none', display: 'inline-block', boxShadow: `0 4px 30px ${COLORS.rosePink}50` }}>
          Enroll Now — From ${tiers.find((t: any) => t.id === selectedTier)?.price || 297}
        </a>
      </section>
      ...
      {showCheckout && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: 20 }}>
          <div style={{ background: COLORS.pureWhite, borderRadius: 24, padding: 40, maxWidth: 480, width: '100%' }}>
            ...
            <form onSubmit={(e: any) => { e.preventDefault(); alert('Stripe checkout would open here') }}>
              <input type="text" placeholder="Full Name" required style={{ width: '100%', padding: 14, borderRadius: 12, border: '2px solid #eee', marginBottom: 12, fontSize: 16 }} />
              <input type="email" placeholder="Email Address" required style={{ width: '100%', padding: 14, borderRadius: 12, border: '2px solid #eee', marginBottom: 12, fontSize: 16 }} />
              <button type="submit" style={{ width: '100%', padding: 18, borderRadius: 12, background: COLORS.rosePink, color: COLORS.pureWhite, fontWeight: 700, fontSize: 18, border: 'none', cursor: 'pointer' }}>
                Pay ${tiers.find((t: any) => t.id === selectedTier)?.price} Securely →
              </button>
            </form>
            <button onClick={() => setShowCheckout(false)} style={{ width: '100%', marginTop: 12, padding: 12, background: 'transparent', color: COLORS.midGrey, border: 'none', cursor: 'pointer', fontSize: 14 }}>
              Cancel
            </button>
          </div>
        </div>
